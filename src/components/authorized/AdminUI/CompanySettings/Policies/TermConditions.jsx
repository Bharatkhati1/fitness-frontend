...
const TermConditions = () => {
  ...
  const fetchPPDetails =async()=>{
    try {
      const res = await adminAxios.get(adminApiRoutes.get_policy_details("terms-conditions"))
        ...
        <div className={`card ${isEdit ? "editing" : ""}`}>
          <div className="card-header d-flex justify-content-between">
            <h4 className="card-title">
              {isEdit ? "Edit Terms & Conditions" : "Terms & Conditions"}
            </h4>
          ...
          <div className="card-footer border-top">
            <button
              ...
              disabled={loading}
            >
              {loading ? "Saving..." : "Update Terms & Conditions" }
            </button>
          </div>
        </div>
      </div>
